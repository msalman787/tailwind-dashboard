            ...
            {data.map((item:any,index:number) =>{
              return (
                <li key={index} className='bg-gray-50 hover:bg-gray-100 cursor-pointer border rounded-lg my-3 p-2 grid md:grid-cols-4 sm:grid-cols-3 grid-cols-2 items-center justify-between'>
                  ...
                  <p className='pl-4'>{`$${item.total}`}</p>
                  </div>
                  <p className={'text-gray-600 sm:text-left text-right'}>
                    <span className={item.status =="Processing"?'bg-green-200 p-2 rounded-lg' : item.status =="Completed" ? 'bg-blue-200 p-2 rounded-lg' : "bg-yellow-200 p-2 rounded-lg"}>
                      {item.status}
                    </span>
                  </p>
